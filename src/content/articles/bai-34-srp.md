...
Có một câu hỏi mà người có kinh nghiệm hay hỏi người mới khi review code: *"Class này tồn tại để làm gì?"*

Câu hỏi đó nghe đơn giản. Nhưng hầu hết người mới không trả lời được trong một câu. Họ sẽ nói: *"Ờ, nó xử lý appointment — validate input, lưu database, gửi notification, ghi log..."* Đó là dấu hiệu đầu tiên của vấn đề.
...
Lấy ví dụ cụ thể từ HMS. Trong một `AppointmentService` lớn, bạn đang phục vụ nhiều "chủ" khác nhau:
...
Thử xem một ví dụ thực tế. Notification team yêu cầu thêm SMS. Bạn mở `AppointmentService` ra sửa:
...
Bạn chỉ thêm ba dòng liên quan đến SMS. Nhưng để làm điều đó, bạn phải:
...
## Cách nhận ra bạn đang vi phạm SRP

Không cần nhớ định nghĩa phức tạp. Chỉ cần để ý ba dấu hiệu:

**Dấu hiệu 1 — "God class":** Class bạn có tên quá generic như `AppointmentManager`, `UserHandler`, `SystemService`. Generic name thường là dấu hiệu class đang làm quá nhiều việc — vì bạn không nghĩ ra tên cụ thể hơn được.

**Dấu hiệu 2 — Nhiều loại dependency:** Class bạn inject vừa repository, vừa emailService, vừa redisTemplate, vừa auditRepository. Nếu dependencies thuộc về nhiều lĩnh vực khác nhau, class đang phục vụ nhiều "chủ" khác nhau.

**Dấu hiệu 3 — Test khó setup:** Khi viết unit test cho class đó, bạn phải mock quá nhiều thứ không liên quan. Để test booking logic, bạn phải setup emailService mock, smsService mock, auditRepository mock — dù chúng không liên quan gì đến thứ bạn đang test.

---

## SRP không yêu cầu bạn tạo class cho từng dòng code

Đây là điểm quan trọng cần nhấn mạnh: tách quá nhỏ cũng là một vấn đề.

Nếu bạn tách `AppointmentEmailSender`, `AppointmentSmsSender`, `AppointmentPushNotificationSender` thành ba class riêng, rồi một ngày PM yêu cầu "khi gửi notification thì log lại xem kênh nào bị fail" — bạn phải sửa ba class đó theo cùng một lý do. Lúc đó tách quá nhỏ đã tạo ra sự phức tạp không cần thiết.
...
Lần tới khi bạn viết xong một class, hỏi: *"Ai sẽ yêu cầu mình sửa class này? Business team? Ops team? Security team?"* Nếu câu trả lời là nhiều hơn một nhóm — tách ra đi. Bạn đang nợ chính bạn trong tương lai.
