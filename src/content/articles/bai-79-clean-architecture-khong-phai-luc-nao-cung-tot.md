...
Sau khi đọc xong Robert Martin hay một series trên Medium về Clean Architecture, có một cảm giác rất thực: *mình phải refactor lại toàn bộ project.* Use cases, entities, ports, adapters, dependency inversion ở mọi layer — tất cả.
...
Khi `AppointmentService` import trực tiếp `JpaRepository`, gọi `RedisTemplate`, và parse `HttpServletRequest` trong cùng một method — thì business logic của bạn bị trói vào Spring, JPA, và Redis. Muốn test logic đặt lịch thì phải spin up database. Muốn swap Redis ra thì phải đụng vào business code.
...
Implement đúng Clean Architecture cho một feature đặt lịch trong HMS có nghĩa là bạn cần:
...
Và quan trọng hơn: overhead đó có payoff khi bạn thực sự cần swap implementation. Nhưng trong 99% project, bạn không swap từ JPA sang Mongo, không swap từ Keycloak sang Auth0. Bạn biết ngay từ đầu stack của mình là gì và nó sẽ không thay đổi trong vòng đời của project.

---

## Gradient, không phải binary

Điều mà nhiều người miss là Clean Architecture không phải on/off switch. Nó là một spectrum, và bạn chọn điểm nào trên spectrum đó tùy vào nhu cầu thực tế.
...
Vấn đề không phải "có nên dùng Clean Architecture không" mà là "bạn đang ở đâu trên spectrum đó và nó có phù hợp với complexity thực tế của project không."

---

## Dấu hiệu bạn đang over-architect

Bạn implement một abstraction layer và không thể đặt tên cho vấn đề cụ thể nó giải quyết.

Bạn viết một `UseCase` class chỉ để delegate sang `Service` mà không có business logic gì trong `UseCase` đó.

Bạn có `DoctorRepository` interface với một implementation duy nhất là `DoctorJpaRepository` và không có kế hoạch nào để thêm implementation thứ hai.

Những dấu hiệu đó không có nghĩa là bạn sai hoàn toàn — chúng có nghĩa là abstraction đang chạy trước nhu cầu.

---

## Điều thực sự quan trọng

Dù bạn chọn architecture nào, một thứ không thay đổi: **business logic không được lẫn với infrastructure concern trong cùng một method.**
...
Clean Architecture hay không, principle này vẫn đúng. Và nếu bạn giữ được điều đó, bạn đã ở 80% của điểm đến rồi.

---

## Takeaway

Trước khi refactor project sang Clean Architecture, hỏi: *"Vấn đề cụ thể mình đang gặp là gì — test khó, swap infrastructure, hay business logic đang bị buried?"* Câu trả lời đó sẽ cho bạn biết bao nhiêu architecture là đủ.
