...
Bạn chạy một query, nó lấy dữ liệu về đúng nhưng khó mà predict nó sẽ nhanh hay chậm từ nhìn SQL. Có lúc query đơn giản mà chậm, có lúc query phức tạp mà nhanh.

Cái bạn cần là cách nhìn vào "kế hoạch" mà MySQL dự định sẽ thực thi query — gọi là **Execution Plan**.

EXPLAIN là công cụ đó. Nó không chạy query, nó chỉ nói — "Mình sẽ làm gì nếu chạy query này". Dựa vào đó, bạn có thể biết có cần optimize hay không.
...
Nếu bạn nhìn thấy `ALL`, đó là dấu hiệu query có vấn đề.
...
Khi bạn tạo index hoặc mỗi khoảng thời gian, MySQL chạy `ANALYZE TABLE` để update statistics.

Nếu statistics lỗi thời, estimate sẽ lệch. Ví dụ bạn insert 50k rows hôm nay nhưng chưa chạy ANALYZE lại, MySQL statistics vẫn nghĩ table có 100k rows. Nó sẽ estimate sai.
...
EXPLAIN là cái mirror cho bạn nhìn MySQL sẽ làm gì. Lần tới query chậm, chạy EXPLAIN trước, nhìn `type` (là ALL không?), `rows` (con số đáng kính hay không?). Thường vấn đề nằm ở đó.
