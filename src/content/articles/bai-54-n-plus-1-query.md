...
Bạn load 100 appointments, mỗi cái appointment có một doctor. Logic đơn giản:
...
Bạn nói cần ngồi kỹ log để nhận ra. Tôi sẽ teach bạn cách spotted mặt nó ngay lập tức.
...
Lý do: optimize case "bạn chỉ cần appointment, không cần doctor". Nhưng nếu bạn loop appointments rồi access doctor, cái này thành vấn đề.
...
Nếu bạn thấy cùng query SELECT chạy rất nhiều lần với bind params khác nhau → **N+1 spotted**. Cái này không xảy ra nếu eager load (JOIN).
...
Nếu bạn dùng logging framework show SQL, thường nó sẽ show:
...
Nếu bạn không muốn JOIN (vì query sẽ duplicate appointment rows nếu doctor có nhiều appointments), dùng batch fetching:
...
Nếu bạn không cần object model phức tạp, dùng DTO:
