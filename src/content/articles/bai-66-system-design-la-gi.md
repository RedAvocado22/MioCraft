...
Có một câu hỏi phỏng vấn mà rất nhiều dev có kinh nghiệm hay hỏi người mới: *"Thiết kế một hệ thống đặt lịch khám bệnh — bạn sẽ bắt đầu từ đâu?"*

Câu trả lời phổ biến nhất từ sinh viên: *"Mình sẽ tạo bảng `Appointment`, bảng `Doctor`, bảng `Patient`..."*
...
Hãy nghĩ về HMS của bạn. Ở tầng thứ nhất là code: Spring Boot service, JPA repository, REST controller. Đây là thứ bạn đang xây dựng mỗi ngày — logic, validation, transformation.
...
Khi thiết kế một tính năng, người có kinh nghiệm sẽ không chỉ hỏi "logic này đúng chưa?" — họ còn hỏi:
...
Những câu hỏi này không có câu trả lời "đúng" hay "sai" theo nghĩa tuyệt đối. Chúng có **trade-off** — bạn tăng availability thì phải hi sinh consistency, bạn tăng throughput thì phải tăng complexity. System Design là kỹ năng nhìn thấy những trade-off đó và chọn cái phù hợp với context.

---

## Tại sao sinh viên thường bỏ qua tầng này

Vì trường không dạy. Và vì trong project sinh viên, hệ thống chạy trên máy local với một mình bạn dùng — không có concurrent user, không có hardware failure, không có traffic spike. Những vấn đề mà System Design giải quyết chỉ xuất hiện ở scale nhất định.

Nhưng đây là cái bẫy: đến khi bạn thật sự gặp những vấn đề đó, cost để fix architecture đã cao hơn rất nhiều so với cost để thiết kế đúng từ đầu.

Phần 8 sẽ đi qua những building block cơ bản nhất: từ functional vs non-functional requirements, monolith vs microservices, API design, load balancer, message queue, rate limiting, circuit breaker, đến CAP theorem. Không phải để bạn trở thành system architect ngay lập tức — mà để bạn bắt đầu nghĩ ở đúng tầng khi thiết kế.

---

## Takeaway

Lần tới khi bạn bắt đầu một feature mới, trước khi tạo entity hay viết service, hãy hỏi: *"Nếu feature này có 1000 người dùng cùng lúc, điều gì sẽ break đầu tiên?"* — câu hỏi đó là entry point của System Design thinking.
