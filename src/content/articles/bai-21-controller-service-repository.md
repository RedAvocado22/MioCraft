...
Tuần đầu tiên: `AppointmentController` nhận request, `AppointmentService` xử lý logic, `AppointmentRepository` truy vấn database. Ba layer rõ ràng, đẹp như sách giáo khoa. Người có kinh nghiệm nhìn vào gật đầu.

Sáu tháng sau: `AppointmentService` có 800 dòng. Trong đó có logic validate, logic tính phí, logic gửi notification, logic check insurance, logic cập nhật doctor schedule — tất cả nằm trong cùng một class. Không ai dám sửa vì không ai hiểu hết nó làm gì. Và bạn biết điều kỳ lạ không? Kiến trúc vẫn đúng theo sách — vẫn ba layer, vẫn Controller/Service/Repository. Nhưng nó đã thối rữa từ bên trong mà không ai nhận ra.
...
Nhưng kiến trúc này không nói gì về việc chia business logic như thế nào *bên trong* Service layer. Nó không ngăn bạn nhét toàn bộ hệ thống vào một `AppointmentService`. Không có quy tắc nào trong Layered Architecture bảo bạn "đây, một service chỉ được phép làm một loại việc." Nó chỉ nói Controller không query DB trực tiếp — hết.
...
Khi bạn thấy một service 800 dòng, phản xạ đầu tiên thường là "dài quá, cần tách ra." Nhưng đó là nhìn vào triệu chứng, không phải nguyên nhân.
...
Nhìn vào cái này và hỏi: *nếu logic tính insurance thay đổi, mình cần sửa file nào?* `AppointmentService`. *Nếu cách gửi notification thay đổi?* `AppointmentService`. *Nếu logic quản lý slot của doctor thay đổi?* `AppointmentService`.
...
Lần tới khi bạn định thêm một method vào một service đang có sẵn, dừng lại và hỏi: *method này giải quyết vấn đề của ai — Appointment, hay của Insurance, hay của Notification?* Nếu câu trả lời không phải Appointment, nó không thuộc về đây.
