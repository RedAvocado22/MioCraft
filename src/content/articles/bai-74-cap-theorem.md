...
CAP Theorem nói rằng trong một distributed system — hệ thống có nhiều node giao tiếp qua network — bạn chỉ có thể đảm bảo được hai trong ba thuộc tính sau:
...
Network partition không phải khả năng lý thuyết — nó là thực tế. Bất kỳ hệ thống nào có nhiều server thì sớm muộn sẽ có lúc hai server không nói chuyện được với nhau: network cable hỏng, switch bị lỗi, datacenter bị ngắt kết nối. Vì vậy, **P là bắt buộc**. Bạn không thể chọn "mình sẽ không có partition" — bạn chỉ chọn cách hệ thống react khi partition xảy ra.

Điều đó có nghĩa lựa chọn thực sự là: **khi có network partition, bạn hy sinh C hay A?**

**CP — Consistency over Availability:** Khi partition xảy ra, hệ thống từ chối serve request thay vì risk trả về data không nhất quán. "Mình không biết câu trả lời chính xác, nên mình sẽ không trả lời." Banking system thuộc loại này — thà từ chối transaction còn hơn risk double-spend.

**AP — Availability over Consistency:** Khi partition xảy ra, hệ thống vẫn serve request nhưng có thể trả về data cũ hoặc data khác nhau từ các node khác nhau. "Mình sẽ trả lời, nhưng có thể không phải thông tin mới nhất." DNS, social media feed, shopping cart thuộc loại này — thà show data hơi cũ còn hơn bị lỗi.
...
Hầu hết database hiện đại cho phép bạn **configure** behavior — và behavior đó có thể khác nhau giữa các operation, thậm chí là giữa các request.

MySQL với replication: nếu bạn đọc từ primary, bạn có strong consistency. Nếu bạn đọc từ replica, bạn có eventual consistency — replica có thể lag vài milliseconds so với primary. Đây là lý do tại sao trong nhiều hệ thống, write và critical read đi đến primary, còn non-critical read (analytics, reports) đi đến replica.

Redis Cluster: khi có network partition, Redis có thể lose writes để đảm bảo cluster vẫn available. Nếu bạn dùng Redis cho slot booking — như HMS đang làm — bạn cần biết rằng trong partition scenario hiếm gặp, một số write có thể bị mất.
...
Đây là trade-off quan trọng hơn trong ngày thường: bạn có muốn mọi write phải đồng bộ đến tất cả replica trước khi return (high consistency, high latency) hay return ngay sau khi write đến primary (low latency, eventual consistency)?

Trong HMS, khi bệnh nhân tạo appointment, bạn muốn low latency hơn hay absolute consistency? Nếu primary return success nhưng replica chưa sync và server primary chết ngay lúc đó — appointment đó có bị mất không? Đây là câu hỏi mà database configuration của bạn trả lời.
...
**Appointment booking** — cần strong consistency. Không thể có hai người cùng giữ một slot. Redis Lua script của bạn đang handle điều này ở tầng application. Ở tầng database, appointment write phải đến primary.
...
CAP Theorem không phải thứ bạn đọc thuộc để trả lời phỏng vấn. Nó là mental model để ra quyết định thiết kế:
...
Lần tới khi chọn database, caching strategy, hoặc replication config, hãy hỏi: *"Với data này, nếu mình phải chọn giữa trả về data cũ và từ chối trả lời, mình chọn gì?"* — câu trả lời đó define consistency requirement của bạn, và từ đó define architecture đúng.

---

*Bài tiếp theo: Microservices không phải phép màu giúp bạn scale*
