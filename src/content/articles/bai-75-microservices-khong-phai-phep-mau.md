---
title: "Microservices không phải phép màu giúp bạn scale"
...
Microservices không làm hệ thống scale tốt hơn. Microservices giúp bạn **scale từng phần độc lập** — nhưng chỉ khi bạn đã hiểu rõ mình muốn scale cái gì và tại sao.

---

## Cái mà mọi người nghe được vs. cái thực sự đúng

Khi đọc về Netflix hay Amazon dùng microservices để handle hàng triệu request mỗi giây, não bạn tự động rút ra kết luận: *microservices = scale được.*
...
- **Deploy independence:** Khi bạn muốn deploy `AppointmentService` mà không phải test lại toàn bộ hệ thống
...
Trong microservices, cùng flow đó bạn cần xử lý:
...
**Vấn đề là tổ chức, không phải performance.** Team bạn quá lớn để làm việc trên một codebase mà không giẫm lên nhau. Khi đó, boundary giữa services cũng là boundary giữa team — và đó là thứ microservices thực sự cho bạn.
...
**Failure domain cần isolation thật sự.** Nếu `ReportingService` đang chạy một báo cáo nặng mà bạn cần đảm bảo nó không ảnh hưởng đến booking — đó là lý do kỹ thuật thật sự.
...
Trước khi nghĩ đến microservices, hãy hỏi: *team mình có bao nhiêu người, và bottleneck thực sự của mình là gì?* Nếu câu trả lời là "mình code một mình và app chậm" — microservices không phải thứ bạn cần.
