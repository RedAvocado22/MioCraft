---
title: "Debug chậm không phải vì mày dở — mày đang debug sai cách"
...
Đây không phải vì mày không đủ giỏi. Đây là vì mày đang debug theo instinct thay vì theo method.

Senior debug nhanh không phải vì họ nhìn vào code và biết ngay vấn đề ở đâu. Họ nhanh vì họ có một approach có hệ thống — và approach đó dramatically reduce search space trước khi bắt đầu nhìn vào code cụ thể.

---

## Tại sao "thêm print và chạy lại" không scale

Approach này có một vấn đề cơ bản: nó là **local search** — mày đang tìm kiếm từng điểm một mà không có hypothesis về bug ở đâu. Trong một codebase nhỏ, điều này có thể work. Trong một system phức tạp hơn, số điểm có thể là bug là vô hạn, và mày có thể mất nhiều giờ chỉ để reach đúng chỗ.

Debug hiệu quả là về **narrowing the search space** trước tiên, rồi mới đi vào detail.

---

## Framework debug của senior

**Bước 1: Reproduce trước tiên**

Trước khi làm bất cứ điều gì, đảm bảo mày có thể reproduce bug một cách consistent. Bug không reproduce được là bug mày chưa hiểu.
...
Dựa trên error message và symptoms, hãy form một hypothesis về bug ở đâu và tại sao. Hypothesis không cần phải đúng — nhưng nó cho mày một điểm để start và một cách để test.

*"Dựa vào NullPointerException ở dòng này, hypothesis của tao là `appointment.getPatient()` đang return null. Điều đó có thể xảy ra nếu appointment được load mà không fetch patient relationship."*

**Bước 4: Test hypothesis với minimum effort**

Cách nhanh nhất để test hypothesis là gì? Không phải thêm print mọi chỗ — mà là kiểm tra cụ thể hypothesis của mày.
...
**Bước 5: Nếu hypothesis sai, update model của mày**

Hypothesis sai không phải failure — nó là information. Mày vừa loại được một khả năng. Update understanding của mày về system và form hypothesis tiếp theo.

---

## Những bug pattern phổ biến trong Spring Boot

Biết những bug pattern phổ biến giúp mày form hypothesis nhanh hơn:
...
**N+1 query:** Mày load một list entities rồi access lazy relationship trong loop. Dùng `@EntityGraph` hoặc `JOIN FETCH` để fetch relationship trong một query.
...
`System.out.println` debugging là fine cho quick checks. Nhưng khi bug phức tạp hơn, debugger cho mày nhiều hơn:
...
Đây là technique tao thấy actually work: giải thích bug cho một người khác (hoặc một con vịt cao su, hay thậm chí một AI). Không phải để người đó giải quyết — mà vì process giải thích buộc mày phải articulate assumptions của mày, và trong quá trình đó mày thường tự tìm ra vấn đề.

*"Bug là: appointment booking thành công nhưng slot count không decrease. Tao nghĩ là Redis Lua script chạy sai. Nhưng khi tao check... hm, thật ra script chạy đúng nhưng tao đang call nó với wrong key format..."*
...
Khi mày thấy mình đã debug hơn 30 phút mà không có progress, đó là dấu hiệu để step back và re-examine hypothesis của mày từ đầu, không phải thêm nhiều print statement hơn.
