---
title: "Đừng nuốt lỗi — hệ thống sẽ trả giá thay bạn"
...
Khi bạn nuốt exception, hệ thống mất đi tín hiệu quan trọng nhất để biết có gì đó đang sai. Log không có gì. Monitoring không alert. Bệnh nhân không nhận được confirmation. Insurance không được verify. Tất cả xảy ra trong im lặng.

Điều tồi tệ hơn: trong nhiều trường hợp, bạn không chỉ bỏ qua lỗi — bạn đang để hệ thống tiếp tục trong trạng thái không nhất quán. `insuranceService.verify()` fail nhưng appointment vẫn được tạo ra và đánh dấu là verified. Bây giờ data trong database nói dối.

Và khi bug này được phát hiện — thường là bởi người dùng, không phải monitoring — bạn không có log nào để trace back.

## Phân biệt: lỗi nào nên handle, lỗi nào nên propagate

Đây là câu hỏi cốt lõi: không phải mọi exception đều cần propagate lên. Nhưng bạn phải có lý do rõ ràng khi quyết định không làm vậy.
...
Một trong những cách clean nhất để handle lỗi là define exception hierarchy cho domain của bạn.
...
Khi bạn quyết định handle exception thay vì propagate, log phải đủ để reconstruct tình huống sau này.
...
Tìm trong HMS của bạn tất cả các `catch` block. Với mỗi cái, hỏi: "Nếu exception này xảy ra lúc 2 giờ sáng và mình đang ngủ, mình có biết không? Và mình có đủ thông tin để debug sáng hôm sau không?" Nếu câu trả lời là không — đó là bug đang chờ ngày ra đời.
