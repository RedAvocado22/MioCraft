...
## SQL scale được — và bạn có thể chứng minh điều đó
...
Vấn đề của SQL không phải là nó không scale. Vấn đề là **vertical scaling có giới hạn** và **horizontal sharding của SQL phức tạp hơn NoSQL** ở một số chiều. Nhưng khi bạn chưa đến ngưỡng đó thì đây không phải vấn đề của bạn.
...
## Cái bạn đánh mất khi chọn NoSQL
...
- Và bỏ đi ACID transaction — nếu appointment save thành công nhưng update patient record fail thì bạn tự xử lý inconsistency
...
**Document store (MongoDB):** Khi structure của data genuinely khác nhau giữa các record và bạn không muốn 50 nullable column. Ví dụ: product catalog trong e-commerce, mỗi category có attribute khác nhau hoàn toàn.
...
**Wide-column (Cassandra):** Time-series data, event log ở quy mô cực lớn. Bạn cần ghi 100,000 event/giây và query theo time range — Cassandra thắng SQL ở đây.
...
Câu hỏi không phải "SQL hay NoSQL?" mà là "access pattern của mình là gì và data model nào phù hợp?" Với relational data có transaction requirements, SQL vẫn là lựa chọn tốt nhất sau 50 năm — không phải vì nó cũ, mà vì nó đúng cho bài toán đó.
